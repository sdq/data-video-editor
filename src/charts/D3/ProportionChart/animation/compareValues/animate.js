...
const draw = (animation, props) => {
    ...
    if (animation.spec.effect === 'juxtaposition') {
        // superposition animation
                ...
                content.selectAll('circle')
                    ...
                    .duration(animation.duration)
                    .style("fill", (d) => {
                        if ((d[encoding.color.field].toString() !== selectedCategory1.toString()) && (d[encoding.color.field].toString() !== selectedCategory2.toString())) {
                            return "lightgray";
                        }
                    })
                    .style("stroke", "yellow")
                ...
                var text = content.append("text")
                    ...
                    .attr("dx", function(d) {
                        // for (var i=0; i<categories.length; i++){
                        //     if(selectedCategory1.toString() === categories[i]){
                ...
                var text1 = content.append("text")
                    ...
                    .attr("dx", function(d) {
                        // for (var i=0; i<categories.length; i++){
                        //     if(selectedCategory2.toString() === categories[i]){
                        //         return i * 2*chartWidth/(categories.length*2.5) + (chartWidth - 2*chartWidth/(categories.length*2.5) * (categories.length-1))/2 - chartWidth/(categories.length*2.5*3);
                        //     }
                        // }
                        var inner = 0;
    ...
    //     let selectedSeries2 = animation.spec.series2 ? animation.spec.series2 : series[0];
