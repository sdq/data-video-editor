...
const draw = (animation, props) => {
    ...
    if (hasSeries) {
        ...
        if (animation.spec.extreme1 === 'max') {
            ...
            aggregatedDS1.sort(function (a, b) { return a[encoding.y.field] - b[encoding.y.field] }); // min
        }
        let extremeCategory1 = aggregatedDS1[0][encoding.x.field];
        ...
        if (animation.spec.effect === 'superposition') {
            ...
            layer.selectAll('rect')
                ...
                .duration(animation.duration)
                .style("fill", (d, i) => {
                    if (!((d.data.x.toString() === extremeCategory1.toString() && d.series.toString() === selectedSeries1.toString()) || (d.data.x.toString() === extremeCategory2.toString() && d.series.toString() === selectedSeries2.toString()))) {
                        return "lightgray";
                    }
                })
                .style("stroke", "yellow")
