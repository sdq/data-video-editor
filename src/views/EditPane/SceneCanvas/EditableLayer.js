...
export default class EditableLayer extends Component {
    ...
    dragMoving(x,y,e) {
        ...
        let margin = 10;//判定辅助线吸附的margin（略小于显示margin）
        ...
        //动态辅助线    
        //拖拽时生成附近xy点数组，遍历匹配去重得到结果，发送redux?直接return?写进props？
        let x1 = x+w;
        let y1 = y+h;
        for(let i =0,xArray = [],yArray = [],dynamicArray = [];i<xPosArray.length;i++){
        if( Math.abs(x - xPosArray[i]) < margin){x = e.target.attrs.x = xPosArray[i];xArray.push([x,-1]);
            if (xArray.indexOf(x) == -1) {dynamicArray.push([x,-1]);}}
        if( Math.abs(x1 - xPosArray[i]) < margin){x = e.target.attrs.x = xPosArray[i]-w;xArray.push([x+w,-1]);
            if (xArray.indexOf(x+w) == -1) {dynamicArray.push([x+w,-1]);}}
        if( Math.abs(y - yPosArray[i]) < margin){y = e.target.attrs.y = yPosArray[i];yArray.push([-1,y]);
            if (yArray.indexOf(y) == -1) {dynamicArray.push([-1,y]);}}
        if( Math.abs(y1 - yPosArray[i]) < margin){y = e.target.attrs.y = yPosArray[i]-h;yArray.push([-1,y+h]);
            if (yArray.indexOf(y+h) == -1) {dynamicArray.push([-1,y+h]);}}
        this.props.setDynamicAssistLines(dynamicArray);
    }
